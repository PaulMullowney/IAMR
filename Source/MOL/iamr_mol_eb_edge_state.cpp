                      ...
                      Array4<EBCellFlag const> const& flag)
{
    ...
    // ****************************************************************************
    auto extdir_lohi_x = has_extdir_or_ho(h_bcrec.dataPtr(), ncomp, static_cast<int>(Direction::x));
    bool has_extdir_or_ho_lo_x = extdir_lohi_x.first;
    bool has_extdir_or_ho_hi_x = extdir_lohi_x.second;

    auto extdir_lohi_y = has_extdir_or_ho(h_bcrec.dataPtr(), ncomp, static_cast<int>(Direction::y));
    bool has_extdir_or_ho_lo_y = extdir_lohi_y.first;
    bool has_extdir_or_ho_hi_y = extdir_lohi_y.second;

#if (AMREX_SPACEDIM == 3)
    auto extdir_lohi_z = has_extdir_or_ho(h_bcrec.dataPtr(), ncomp, static_cast<int>(Direction::z));
    ...
    else // We assume below that the stencil does not need to use hoextrap or extdir boundaries
    {
        ...
        // ****************************************************************************
        amrex::ParallelFor(xbx, ncomp,
        [q,ccc,flag,umac,xedge,AMREX_D_DECL(fcx,fcy,fcz)]
        AMREX_GPU_DEVICE (int i, int j, int k, int n) noexcept
        {
           Real qs;
           if (flag(i,j,k).isConnected(-1,0,0))
        ...
        // ****************************************************************************
        amrex::ParallelFor(ybx, ncomp,
        [q,ccc,flag,vmac,yedge,AMREX_D_DECL(fcx,fcy,fcz)]
        AMREX_GPU_DEVICE (int i, int j, int k, int n) noexcept
        {
            Real qs;
            if (flag(i,j,k).isConnected(0,-1,0)) {
        ...
        // ****************************************************************************
        amrex::ParallelFor(zbx, ncomp,
        [q,ccc,flag,wmac,zedge,AMREX_D_DECL(fcx,fcy,fcz)]
        AMREX_GPU_DEVICE (int i, int j, int k, int n) noexcept
        {
            if (flag(i,j,k).isConnected(0,0,-1)) {
                Real qs;
